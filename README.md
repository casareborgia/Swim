...
5. **MediaPipe Pose 기반 사이클 정렬 전처리**  
   데크(물 밖) 촬영 시, MediaPipe Pose로 손목 궤적의 스트로크 사이클을 감지하고 매 사이클의 동일한 국면(피크 및 중간점)에 정렬된 키프레임을 프레임 단위로 추출해 판정의 재현성을 비약적으로 높였습니다. (자유형 측면 3회 일치율 86.7% → 96.7%로 상승). 감지율 및 사이클 기준 미달 시 균일 추출로 자동 폴백합니다.
...
[React SPA] 수영 영상 및 정보 접수 폼 → POST /api/submit
                                ↓
[FastAPI]   접수 데이터 수신 → 데크 촬영 시 MediaPipe Pose 사이클 정렬 전처리 기동
                                ↓
[FastAPI]   백그라운드 태스크 기동 (Gemini API 3회 병렬 분석 실행)
                                ↓
[FastAPI]   분석 결과 집계 및 다수결 기반 리포트 초안 생성 → 발송 대기열 등록
                                ↓
[운영자/관리자] 발송 대기열(/admin)에서 생성된 리포트 최종 승인 → 사용자 화면(프론트엔드)에서 즉시 확인 또는 이메일 발송
```

### 디렉토리 구성
* **`server.py`**: FastAPI 백엔드 (업로드 처리, 시간당 IP당 어뷰징 방지 레이트 리미트, 발송 대기열 API, 전문가 검수 대기 UX 상태 API 제공)
* **`pose_keyframes.py`**: MediaPipe Pose (VIDEO 모드) 기반 수영자 포즈 및 스트로크 사이클 감지, 사이클 정렬 키프레임 추출 전처리 모듈
...
ALLOW_DEMO_BYPASS=0                  # demo@example.com 우회 여부 (1=우회허용(시연용), 0=우회미허용(배포용 기본값))
BYPASS_REVIEW=0                      # 리포트 전문가 검수 우회 여부 (1=즉시열람(시연용), 0=검수승인필수(배포용 기본값))
```
